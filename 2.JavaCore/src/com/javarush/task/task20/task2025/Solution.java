...
public class Solution {
    ...
    public static long[] getNumbers(long N) {
        ...
        long[] result;
        if (N <= 0) {  // если число ненатуральное(т.е. >=0), то возвращаем пустой массив
            return result = new long[0];
        }
        ArrayList<Long> list = new ArrayList<>();
        // Необходимые числа называются числами Армстронга, добавляем все числа в массив, которые влязт в лонг ниже:
        long[] armstrongNumbers = {1, 2, 3, 4, 5, 6, 7, 8, 9, 153, 370, 371, 407,
        ...
        for (int i = 0; i < armstrongNumbers.length; i++) {
            if (armstrongNumbers[i] < N) { // добавляем в список все числа армстронга, которые меньше N
                list.add(armstrongNumbers[i]);
            }
        }
        result = new long[list.size()];
        for (int i = 0; i < result.length; i++) {
            result[i] = list.get(i); // добавляем в результат все полученные числа,которые хранятся в листе
        }
